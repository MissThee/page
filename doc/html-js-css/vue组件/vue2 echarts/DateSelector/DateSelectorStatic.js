...
export const formatStrForComponent = 'yyyy-MM-dd'
export const formatStrForDayJs = 'YYYY-MM-DD'

export const btnDate= [
...
// date 为使用指定时间计算结果
export const getParamByDateType = (dateType, date, needAuxiliaryParam = false, dayOffset =0) => {
    const btnTmp = btnDate.find(e => e.type === dateType)
    if (!btnTmp) {
        throw new Error('invalid dateType')
    }
    // 默认昨天
    const currentDay = date ? dayjs(date) : dayjs().add(dayOffset, 'day')
    // 请求用的参数值
    let time_value
    // 辅助展示的参数值
    ...
    const valueForComponent = currentDay.format(formatStrForDayJs)
...
export const getDateTextByParam = (param) => {
    let result = ''
