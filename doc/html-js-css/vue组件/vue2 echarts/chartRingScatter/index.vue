    ...
    <div class="body" :style="{height: totalHeight +'px'}">
      ...
      <div class="panel" ref="panelRef">
        <div class="ring-wrapper"
             :class="{'ring-wrapper--active':activeId!==null&&activeId===item.id}"
        ...
        >
          ...
          <div class="info" :style="{transform:`translate(0%,-50%) scale(${ Math.min(item.r/(totalHeight/2/2),1)})`}">
            ...
            <div class="percent">P:{{ percentDigitalFormatter(item.positivePercent) }}%</div>
            <div class="percent">N:{{ percentDigitalFormatter(item.negativePercent) }}%</div>
          </div>
        </div>
        ...
        >
          <table v-if="dataForHover">
            <tr>
              ...
              <td>{{ percentDigitalFormatter(dataForHover.percent) || 0 }}%</td>
            </tr>
            <tr class="split"/>
            <tr v-for="p in settingData" :key="p.key">
              <td>
...
import setBubbleInRect from "./setBubbleInRect.js";
...
export default {
  ...
  data() {
    return {
      ...
      popoverPosition: {x: null, y: null, isLeftSide: false}
    }
  },
  methods: {
    percentDigitalFormatter,
    setDataForShow() {
      const dataLength = this.data.filter(e => e.total).length || this.data.length
      if (!dataLength) {
        this.dataForShow = []
        return
      }
      let dataForShowTmp = setBubbleInRect(this.data, totalHeight / 2 / 2, totalHeight, this.$refs.panelRef.clientWidth)
      const allBubbleRect = {
        left: this.$refs.panelRef.clientWidth,
        right: 0,
        top: 0,
        bottom: 0,
      }
      ...
      // 总宽度超出时，按比例减小圆半径
      if (allBubbleWidth > this.$refs.panelRef.clientWidth) {
        const scaleRate = this.$refs.panelRef.clientWidth / allBubbleWidth
        dataForShowTmp.forEach(e => {
          if (e.y > 0) {
          ...
          e.x = this.$refs.panelRef.clientWidth / 2 + (e.x - this.$refs.panelRef.clientWidth / 2) * scaleRate
          e.r = e.r * scaleRate
        })
      }
      const scaleRate = this.$refs.panelRef.clientWidth / allBubbleWidth
      ...
      // 修正中心对齐
      dataForShowTmp.forEach(e => {
        e.x = e.x - (Math.abs(allBubbleRect.right - this.$refs.panelRef.clientWidth / 2) - Math.abs(this.$refs.panelRef.clientWidth / 2 - allBubbleRect.left)) / 2 * scaleRate
      })
      this.dataForShow = dataForShowTmp
    },
    ringEnterHandler(item, e) {
      this.dataForHover = item
      setTimeout(() => {
        if (e) {
          const popoverRect = this.$refs.popoverRef.getBoundingClientRect()
          const panelRect = this.$refs.panelRef.getBoundingClientRect()
          this.popoverPosition = {
            x: item.x + e.offsetX - item.r,
            y: Math.min(-item.y + e.offsetY - item.r, totalHeight / 2 - popoverRect.height),
            isLeftSide: false,
          }
          this.popoverPosition.isLeftSide = panelRect.width - this.popoverPosition.x < popoverRect.width
        }
      })
    },
    ringClickHandler(item) {
      this.$emit('click', item)
    }
  },
  mounted() {
    this.setDataForShow()
    window.addEventListener('resize', this.setDataForShow)
  },
  beforeDestroy() {
    window.removeEventListener('resize', this.setDataForShow)
  },
  watch: {
    data: {
      handler() {
