        ...
        >
          <div class="wrapper">
            ...
            <div :style="{
                    ...
                    width:`calc((100% - ${labelWidth}px - ${percentWidth}px - ${lineMargin + (lineMarginLeft||0) + (lineMarginRight||0)}px))`
                 }">
              <div class="line" :style="{width:`${(maxValue?item[props.mainProp]/maxValue:0)*100}%`, backgroundColor:lineColor}">
                <div class="percent" :style="{width:percentWidth+'px',right:-(lineMargin)+'px'}">
                  <template v-if="props.showNumAndPercent">
                    <AnimateNumber :duration="300" :num="item.value" :formatter="e=>e"/>
                    <template v-if="item.percent!==null && item.percent!==undefined">
                      <span style="margin-right: 5px">,</span>
                      <AnimateNumber :duration="300" :num="item.percent" :formatter="percentDigitalFormatter"/>
                      <span>%</span>
                    </template>
                  </template>
                  <template v-else>
                    <AnimateNumber :duration="300" :num="item[props.mainProp]" :formatter="props.mainProp==='percent'?percentDigitalFormatter:e=>e"/>
                    <span v-if="props.mainProp==='percent'">%</span>
                  </template>
                </div>
              </div>
            </div>
...
const percentWidth = props.showNumAndPercent ? 100 : 35
const lineMargin = 4
...
const lineColor = '#466AF2'

const getMaxLineValue = (dataArr: ListProgressData[]) => dataArr.reduce?.((m, item) => Math.max(item[props.mainProp], m), 0)
...
const updateLabelWidth = () => {
  nextTick(() => {
    ...
    labelWidth.value = Math.min(labelWidth.value, (mainPanelRef.value.getBoundingClientRect().width / 2) || 120)
