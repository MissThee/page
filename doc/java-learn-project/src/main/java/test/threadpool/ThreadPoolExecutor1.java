...
//  ExecutorService为ThreadPoolExecutor继承的接口
...
//3	CachedThreadPool,ScheduledThreadPool 	队列长度固定，允许的创建线程数量为Integer.MAX_VALUE   会创建大量的线程，从而导致 OOM
  ...
  private static void runTest() throws InterruptedException {
    ...
    for (int i = 0; i < 15; i++) {
      executorService.submit(runnable);//此处同时处理任务数为2，队列数为5，之后的任务会被拒绝，并抛出异常，直接结束主线程。但线程池仍会继续执行任务，并且不会自动关闭
    }
  }
  ...
  private static void shutdownThreadPool() throws InterruptedException {
    //executorService.shutdown将线程池状态置为SHUTDOWN，并不会立即停止：
    //1.停止接收外部submit的任务
    //2.内部正在跑的任务和队列里等待的任务，会执行完
    //3.等到第二步完成后，才真正停止
    executorService.shutdown();

    //executorService.awaitTermination当前线程阻塞，直到满足其中一条：
    ...
    //executorService.shutdownNow将线程池状态置为STOP。企图立即停止，事实上不一定：
