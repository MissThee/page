...
public class DeptController {
    ...
    @HystrixCommand(commandKey = "dept-8001-get", fallbackMethod = "processHystrix_Get") // 【Hystrix1/2】调用抛出异常时调用fallbackMethord指定方法。此种写法仅示例，实际使用不会每种方法单独写HystrixCommand，原因会产生大量冗余方法，应直接在所调用接口处写熔断处理
    public Dept get(@PathVariable("id") int id) {
        System.out.println("dept-8001-get");
    ...
    @HystrixCommand(commandKey = "dept-8001-list")
    ...
    public Dept get1(@PathVariable("id") int id) {
        System.out.println("请求 8001/dept/get/{id}");
        return new Dept().setId(1).setDb_source("8001").setName("8001");
    }
}
//@HystrixCommand注解的各个参数：
//1：commandKey：配置全局唯一标识服务的名称，比如，库存系统有一个获取库存服务，那么就可以为这个服务起一个名字来唯一识别该服务，如果不配置，则默认是@HystrixCommand注解修饰的函数的函数名。commandKey相同的会共用一个图，不配置此值则世界使用方法名作为commandKey
