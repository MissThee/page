<template>
  <div>
    ...
    <el-table :height="tableAutoHeight" :data="tableData" border stripe header-cell-class-name="custom-header-cell" style="width: 100%">
      ...
      <el-table-column prop="resultTypeName" width="80" align="center" sortable label="分类">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" fixed="right" width="200" label="操作">
        <template slot-scope="scope">
          <el-dropdown trigger="click" style="margin-right: 10px" v-if="letClueIsEditable(scope.row.resultTypeId)" @command="submitProcessingType">
            <el-button type="warning" size="mini" plain class="custom-button-in-table" icon="el-icon-edit">分类</el-button>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-for="item in typeOptions" :command="scope.row.id+','+item.id">
                {{item.name}}
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      DataDictionaryApi.getDictionary({ listUrl: '/dic/resulttype/all', isDelete: false })
        .then(({ data }) => {
          ...
          this.typeOptions = data.data.dicCommonList.filter(item => item.isInLetClue);
        });
      // 线索来源
    ...
    methods: {
      ...
      submitProcessingType(idAndTypeId) {
        if (idAndTypeId.indexOf(',') < 0) {
          return;
        }
        let id = idAndTypeId.split(',')[0];
        let typeId = idAndTypeId.split(',')[1];
        let typeName;
        ...
        this.$confirm('确定分类为 <span style=\'color: blue\'>' + typeName + '</span> ?', '', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          ...
          .then(() => {
            ...
            // console.log('submitProcessingType', id, typeId);
