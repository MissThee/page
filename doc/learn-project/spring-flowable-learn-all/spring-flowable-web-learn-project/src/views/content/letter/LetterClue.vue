<template>
  <div style="width:800px;margin:0 auto; " class="card-style">
    <el-form :model="letClueForm" label-width="80px" size="mini" :disabled="!canEdit">
      ...
      <el-form-item v-if="canEdit" style="text-align: center">
        <el-button type="primary" size="large" style="margin-left: -80px" icon="el-icon-circle-check" @click="submitGroup" v-if="">提交</el-button>
      </el-form-item>
    </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitGroup() {
        let submitAddFunc;
        let submitUpdateFunc;
        if (this.$route.meta.title === '案件管理') {
          submitAddFunc = caseManagement.casePipeAdd;
          submitUpdateFunc = caseManagement.casePipeAmend;
        } else {
          submitAddFunc = LetterClueApi.addLetClue;
          submitUpdateFunc = LetterClueApi.updateLetClue;
        }
        this.letClueForm.fileList = this.$refs.FileUploader.getFileList();
        if (this.isCreate) {
          submitAddFunc(this.letClueForm)
            .then(({ data }) => {
              showResMsg(data);
          ...
          submitUpdateFunc(this.letClueForm)
            .then(({ data }) => {
              showResMsg(data);
              this.$emit('finishAddOrUpdate', data.result);
            });
