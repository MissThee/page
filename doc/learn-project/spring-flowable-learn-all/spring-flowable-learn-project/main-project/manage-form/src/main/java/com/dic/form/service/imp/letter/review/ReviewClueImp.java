package com.dic.form.service.imp.letter.review;
...
import com.dic.common.config.exception.custom.MyMethodArgumentNotValidException;
import com.dic.common.db.entity.primary.*;
import com.dic.common.tool.MapAndBean;
import com.dic.form.db.mapper.primary.letter.casemanage.CaseManageMapper;
import com.dic.form.db.mapper.primary.letter.clue.LetClueDefendantMapper;
import com.dic.form.db.mapper.primary.letter.clue.LetDefendantMapper;
import com.dic.form.db.mapper.primary.letter.review.ReviewClueMapper;
import com.dic.form.db.mapper.primary.letter.review.ReviewClueUserHistoryMapper;
import com.dic.form.models.dto.letter.review.ReviewClueDTO;
import com.dic.form.models.dto.letter.review.ReviewClueOneDTO;
import com.dic.form.models.dto.letter.review.ReviewGetListResDTO;
import com.dic.form.models.dto.letter.review.ReviewGetOneResDTO;
import com.dic.form.service.interf.letter.review.ReviewClueService;
import com.dic.form.utils.StringUtil;
...
public class ReviewClueImp extends ServiceImpl<ReviewClueMapper, ReviewClue> implements ReviewClueService {
    ...
    public boolean updateOne(String userIdStr, Long id, Map<String, String> form) throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, InstantiationException, MyMethodArgumentNotValidException {
        ...
        if (reviewClueMapper.updateById(reviewClueForUpdate) > 0) {
            ...
            if (flowIsFinish) {
                String letClueId = reviewClue.getLetClueId();
                //审批结束，案件管理插入记录。先设置此线索旧的记录isDead为true，将其标记失效
                QueryWrapper<CaseManage> qw = new QueryWrapper<>();
                qw.eq(CaseManage.LET_CLUE_ID, letClueId);
                caseManageMapper.update(new CaseManage().setIsDead(true), qw);
                caseManageMapper.insert(new CaseManage().setLetClueId(letClueId));
            }
            return true;
        } else {
            return false;
