<template>
  ...
  <div style="text-align: center">
    <span>本页面为托管到github的静态页面，所有登录信息均存放在本地浏览器，无任何后台服务进行存储</span>
    ...
    <el-form style="width: 400px;text-align: left;margin:10px auto;padding:10px;border:1px solid #cccccc;border-radius: 5px">
      ...
      <el-form-item>
        ...
        <el-input v-model="code" placeholder="url"></el-input>
      </el-form-item>
      <el-form-item>
        <a :href="getTokenUrl" target="_blank">
          <el-button type="primary" size="mini" plain>2、下载token文件，拷贝access_token值到以下位置</el-button>
        </a>
        <el-input v-model="token" :placeholder="tokenPlaceHolder"></el-input>
        <div style="color:gray;line-height: 15px">（若下载的文件内容为报错信息，需从第一步重新开始）</div>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" size="mini" plain @click="setAuthToken">3、应用access_token</el-button>
      </el-form-item>
    </el-form>
...
import AuthApi from 'src/docviewer/api/auth-api';
...
export default {
  ...
  created() {
    this.code = this.getAllUrlParams().code || ''
    if (Cookie.isManualLogin.getIsManualLogin() === 'false') {
      this.getAuthToken();
    } else if (Cookie.isManualLogin.getIsManualLogin() === 'true') {
  ...
  methods: {
    ...
    setAuthToken() {
      this.token = this.tokenPlaceHolder || this.token;
      if (this.token) {
        Cookie.token.setTokenValue(this.token);
        AuthApi.getUserInfo()
          .then(({data}) => {
            this.setUser(data);
            this.$notify.success('当前用户：' + this.getUser.login);
            this.tokenPlaceHolder = Cookie.token.getTokenValue();
    ...
    setRepository() {
      this.repository = this.repository || this.repositoryPlaceHolder;
      Cookie.repository.setRepositoryValue(this.repository);
      this.$notify.success('设置成功');
    },
    setDocRoot() {
      this.docRoot = this.docRoot || this.docRootPlaceHolder;
      Cookie.docRoot.setDocRootValue(this.docRoot);
      this.$notify.success('设置成功');
    },
    setRefName() {
      this.refName = this.refName || this.refNamePlaceHolder;
      Cookie.refName.setRefNameValue(this.refName);
      this.$notify.success('设置成功');
    },
    getAuthToken() {
      ...
      if (code) {
        AuthApi.getAccessToken({code: code})
          ...
          .catch((error) => {
            ...
            console.log('AuthApi.getAccessToken 异常', error)
            this.$notify.warning('因跨域问题，需手动设置token');
            // if (error.response) {
    ...
    delCookie() {
      ...
      if (keys) {
        for (let i = keys.length; i--;) {
          document.cookie = keys[i] + '=0;path=/;expires=' + new Date(0).toUTCString(); // 清除当前域名下的,例如：m.exp.cn
          document.cookie = keys[i] + '=0;path=/;domain=' + document.domain + ';expires=' + new Date(0).toUTCString(); // 清除当前域名下的，例如 .m.exp.cn
          // document.cookie = keys[i] + '=0;path=/;domain=exp.cn;expires=' + new Date(0).toUTCString(); // 清除一级域名下的或指定的，例如 .exp.cn
        }
      }
